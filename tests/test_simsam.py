...
class SimsamTests(TestCase):
    ...
    def test_script(self):
        ...
        self.assertEqual(3*len(sample_ids),len(res_sample_ids))

        # same total sequences in sample
        self.assertEqual(res_otu_mtx[:,range(0,27,3)].sum(0),otu_mtx.sum(0))
        self.assertEqual(res_otu_mtx[:,range(1,27,3)].sum(0),otu_mtx.sum(0))
        self.assertEqual(res_otu_mtx[:,range(2,27,3)].sum(0),otu_mtx.sum(0))

    def test_script_nochange(self):
        """ test the whole simsam script, with 0 distance
        """
        qiime_config = qiime.util.load_qiime_config()
        tempdir = qiime_config['temp_dir'] or tempfile.gettempdir()
        maindir = os.path.join(tempdir,
        ...
        # note this requires the same sorting of otus
        self.assertEqual(res_otu_mtx[:,range(0,27,3)],otu_mtx)
        self.assertEqual(res_otu_mtx[:,range(1,27,3)],otu_mtx)
        self.assertEqual(res_otu_mtx[:,range(2,27,3)],otu_mtx)

        # same otu ids
        self.assertEqual(otu_ids, res_otu_ids)


    def test_get_new_otu_id(self):
