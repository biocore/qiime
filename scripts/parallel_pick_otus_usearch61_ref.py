...
script_info['script_usage'].append(
    ("Example",
     ...
     "with reference-based usearch and write the output to the "
     "$PWD/usearch_ref_otus/ directory. This is a closed-reference OTU "
     "picking process. ALWAYS SPECIFY ABSOLUTE FILE PATHS (absolute path "
...
script_info['optional_options'] = [
    ...
    make_option('--max_rejects', default='default',
                help="max_rejects value for uclust, uclust_ref, usearch61, and"
                     ...
                     "(uclust: 500, usearch61: 8 for usearch_fast_cluster "
                     "option, 32 for reference and smallmem options) "
                     "[default: %default]"),
    ...
    make_option('--word_length', default='default',
                help="word length value for uclust, uclust_ref, and "
                     ...
                     "With default setting, will use the setting recommended "
                     "by the method (uclust: 12, usearch: 64, usearch61: 8). "
                     "int value can be supplied to override this setting. "
                     "[default: %default]"),

    make_option('--minlen', default=64, type='int',
                help="Minimum length of sequence allowed for usearch, "
                     "usearch_ref, usearch61, and usearch61_ref. [default: "
                     "%default]"),

    make_option('--usearch_fast_cluster', default=False, action='store_true',
                help="Use fast clustering option for usearch or usearch61_ref "
                     "with new clusters.  --enable_rev_strand_match can not be"
                     " enabled with this option, and the only valid option for"
    ...
    make_option('--usearch61_sort_method', default='abundance', type='str',
                help="Sorting method for usearch61 and usearch61_ref.  Valid "
                     "options are abundance, length, or None.  If the "
                     "--usearch_fast_cluster option is enabled, the only "
                     "sorting method allowed in length. [default: %default]"),

    make_option('--sizeorder', default=False,
                help="Enable size based preference in clustering with "
                     "usearch61. Requires that --usearch61_sort_method be "
                     "abundance. [default: %default]", action='store_true'),
