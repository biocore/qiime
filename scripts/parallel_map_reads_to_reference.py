...
script_info['script_usage'] = [
    ("", "",
     "%prog -i $PWD/query_nt.fasta -r $PWD/refseqs_pr.fasta -o "
     "$PWD/usearch_mapped"),
    ("", "",
     "%prog -i $PWD/query_nt.fasta -r $PWD/refseqs_pr.fasta -o "
     "$PWD/blat_mapped -m blat"),
    ("", "",
     "%prog -i $PWD/query_nt.fasta -r $PWD/refseqs_nt.fasta -o "
     "$PWD/bwa-short_mapped -m bwa-short")]
...
script_info['optional_options'] = [
    make_option('-t', '--observation_metadata_fp', type='existing_filepath',
                help='Path to observation metadata (e.g., taxonomy, EC, etc) '
                     '[default: %default]'),

    make_option('-m', '--assignment_method', type='choice',
                choices=['usearch', 'blat', 'bwa-short'], default="usearch",
                help='Method for picking OTUs.  Valid choices are: {0}. '
                     '[default: %default]'.format(' '.join(['usearch', 'blat',
                                                            'bwa-short']))),
    ...
    make_option('--queryalnfract', type='float', default=0.35,
                help='Min percent of the query seq that must match to consider'
                     ' a match (usearch only) [default: %default]'),

    make_option('--targetalnfract', type='float', default=0.0,
                help='Min percent of the target/reference seq that must match '
                     'to consider a match (usearch only) [default: %default]'),
    ...
    make_option('--max_rejects', type='int', default=32,
                help="max_rejects value to (usearch only) [default: "
                     "%default]"),
