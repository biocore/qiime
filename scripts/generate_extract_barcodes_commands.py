...
from qiime.workflow.util import (print_commands,
                                 ...
                                 WorkflowLogger,
                                 no_status_updates)
from qiime.generate_extract_barcodes_commands import (get_pairs,
                                                      create_commands)
...
script_info['script_usage'].append(("""Example of an input folder of files, with options specified to extract 4 bases from the start and 6 bases from the end of the reads:""",
                                    """The input folder is named 'input_paired_files', the output directory is 'output_folder', and an optional parameter is added of '--input_type barcode_paired_stitched --bc1_len 6 --bc2_len 4'""",
                                    """%prog -i input_stitched_files/ -o output_folder/ --type_of_input folder_of_files --optional_params '--input_type barcode_paired_stitched --bc1_len 6 --bc2_len 4'"""))
script_info['script_usage'].append(("""Example of an input folder of folders (with the filenames having _forward_ and _reverse_ contained the forward and reverse read filenames, respectively), using the extract_barcodes option for paired reads:""",
                                    """The input folder is named 'input_folders', the output directory is 'output_folder'. Additionally, the individual folder names are included in the output folder names, but not the filenames. Note-it is important to pass the --paired_data parameter if paired data are to be used in the extract_barcodes.py commands.""",
                                    """%prog -i input_folders/ -o output_folder/ --paired_data --type_of_input folder_of_folders --optional_params '--input_type barcode_paired_end' --read1_indicator '_forward_' --read2_indicator '_reverse_' --include_input_dir_path --remove_filepath_in_name"""))
...
script_info['optional_options']= [\
    make_option('--type_of_input', type='choice', default='folder_of_files',
        ...
        'will be searched for by this script.'
        ' [default: %default]', choices=['folder_of_files',
        'folder_of_folders']),
    make_option('-p', '--paired_data', action='store_true',
        ...
        '--optional_params parameter. Forward and reverse reads will be '
        'searched for via the --read1 and --read2_indicator parameters. '
        ' [default: %default]', default=False),
    make_option('--optional_params', type='str', default='',
        help='Added optional parameter string to use in extract_barcodes.py, '
        'such as "--rev_comp_bc1" [default: %default]'),
...
def main():
    ...
    if paired_data:
        all_files = get_pairs(all_files, read1_indicator, read2_indicator)
    
    commands = create_commands(all_files, paired_data, base_output_dir,
