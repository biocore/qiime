...
script_info = {}
script_info[
    'brief_description'] = """Summarize taxa and store results in a new table or appended to an existing mapping file."""
...
def main():
    ...
    if upper_percentage is not None and lower_percentage is not None and mapping:
        raise ValueError(
            "upper_percentage and lower_percentage can not be using with mapping file")

    if upper_percentage is not None and (upper_percentage < 0 or upper_percentage > 1.0):
        raise ValueError('max_otu_percentage should be between 0.0 and 1.0')

    if lower_percentage is not None and (lower_percentage < 0 or lower_percentage > 1.0):
