...
script_info['optional_options'] = [
    ...
    make_option('-q', '--queue_name', default=qiime_config['slurm_queue'],
...
def write_sbatch_files(output_dir, commands, run_id, queue_name):
...
def run_commands(output_dir, commands, run_id,
    ...
    """
    """
    job_fps, paths_to_remove = write_sbatch_files(
        output_dir, commands, run_id, queue_name)

    # Call the jobs
    if submit_jobs:
        for job_fp in job_fps:
            qiime_system_call(' '.join(['sbatch', job_fp]))
