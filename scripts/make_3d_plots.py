...
script_info['optional_options'] = [
    ...
    make_option(
        '--add_vectors', type='string', dest='add_vectors', default=None,
    ...
    make_option(
        '--vectors_path', dest='vectors_path', default='vectors_output.txt',
        type='new_filepath',
...
script_info['option_label'] = {'coord_fname': 'Principal coordinates filepath',
                               ...
                               'biplot_output_file':
                               'Output biplot coordinate filepath',
                               'master_pcoa':
                               ...
                               'vectors_path':
                               'Vectors output path calculations',
                               'vectors_axes':
                               'Number of axes to to calculate the vectors.',
...
Valid methods are: " + ', '.join(ellipsoid_methods) + ".")
    ...
    if opts.add_vectors:
        ...
        if len(add_vectors) > 3:
            raise ValueError(
                'You must add maximum 3 columns but %s' %
                opts.add_vectors)
        ...
        if opts.vectors_algorithm:
            ...
            for c in opts.vectors_path:
                if c not in valid_chars:
                    raise ValueError(
                        'vectors_path (%s) has invalid chars' %
                        opts.vectors_path)
    ...
    if opts.taxa_fname is not None:
        # get taxonomy counts
        ...
        data['taxa']['prevalence'] = get_taxa_prevalence(
            data['taxa']['counts'])
    ...
    if action:
        action(
            prefs, data, custom_axes, background_color, label_color, dir_path,
