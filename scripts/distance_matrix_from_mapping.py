    ...
    if ',' not in opts.column:
        ...
        for i in data:
            if column_name not in data[i]:
                raise ValueError, "No column: '%s' in the mapping file. Existing columns are: %s" % (column_name ,data[i].keys())
        ...
        try:
            ...
            raise ValueError, "This script accepts a maximum of 2 colums separated by comma and you passed: %s" % (opts.column)
            
        for i in data:
            if latitude not in data[i] or longitude not in data[i]:
                raise ValueError, "One of these columns or both do not exist: '%s' or '%s' in the mapping file. Existing columns are: %s" % (latitude, longitude, data[i].keys())
                
            try:
                latitudes.append(float(data[i][latitude]))
                longitudes.append(float(data[i][longitude]))
            except ValueError:
                raise ValueError, "All the values in the columnd '%s' & '%s' must be numeric but '%s' has '%s'" % (latitude, longitude, i, data[i][column_name])
