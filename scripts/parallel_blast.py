...
script_info['script_description'] = (
    "This script for performing blast while making use of multicore/"
    "multiprocessor environments to perform analyses in parallel.")
script_info['script_usage'] = []
script_info['script_usage'].append(
    ("Example",
     "BLAST $PWD/inseqs.fasta (-i) against a blast database created from "
     "$PWD/refseqs.fasta (-r). Store the results in $PWD/blast_out/ (-o). "
...
script_info['optional_options'] = [
    make_option('-c', '--disable_low_complexity_filter',
                ...
                help='disable filtering of low-complexity sequences (i.e., '
                     '-F F is passed to blast) [default: %default]'),
    make_option('-e', '--e_value', action='store', type='float',
                default=1e-30, dest='e_value',
                help='E-value threshold for blasts [default: %default]'),
    make_option('-n', '--num_hits', action='store',
                type='int', default=1, dest='num_hits',
                help='number of hits per query for blast results '
                     '[default: %default]'),
    make_option('-w', '--word_size', action='store',
                type='int', default=30, dest='word_size',
                help='word size for blast searches [default: %default]'),
    make_option('-a', '--blastmat_dir', action='store', type='string',
    ...
    make_option('-b', '--blast_db', type='blast_db',
                help='Name of pre-formatted BLAST database. Required if -r is '
                     'not provided.'),
...
def main():
    ...
    if opts.refseqs_path and opts.blast_db:
        option_parser.error('You should provide only a blast db (via -b) '
                            'or a collection of reference sequences (via -r), '
                            'but not both')
