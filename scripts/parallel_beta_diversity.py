...
script_info['script_description'] = (
    "This script performs like the beta_diversity.py script, but is intended "
...
script_info['script_usage'].append(
    ("Apply beta_diversity.py in parallel to multiple otu tables",
     "Apply the unweighted_unifrac and weighted_unifrac metrics (modify with "
     "-m) to all otu tables in rarefied_otu_tables (-i) and write the "
     "resulting output files to bdiv/ (-o, will be created if it doesn't "
     "exist). Use the rep_set.tre (-t) to compute phylogenetic diversity "
     "metrics. ALWAYS SPECIFY ABSOLUTE FILE PATHS (absolute path represented "
...
script_info['output_description'] = (
    "The output of %prog is a folder containing text files, each a distance "
    "matrix between samples.")

script_info['required_options'] = [
    make_option('-i', '--input_path', type='existing_path',
                help='input path, directory for batch processing [REQUIRED]'),
...
script_info['optional_options'] = [
    make_option(
        ...
        help='Beta-diversity metric(s) to use. A comma-separated list should '
             'be provided when multiple metrics are specified. [default: '
             '%default]'),
    ...
    make_option('-f', '--full_tree', action="store_true",
                help='By default, each job removes calls _fast_unifrac_setup '
...
def main():
    ...
    if isfile(input_path):
        ...
        parallel_runner(input_path, output_dir, params,
        ...
        parallel_runner = ParallelBetaDiversityMultiple(
            retain_temp_files=opts.retain_temp_files,
            block=not opts.suppress_blocking)
        parallel_runner(input_fps, output_dir, params)
