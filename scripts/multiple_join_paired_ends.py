...
script_info['script_description']="""The purpose of this script is to automate the creation of join_paired_ends.py commands for multiple files. This script is intended to help in cases of having data already demultiplexed (split up according to SampleID), either as a folder of paired files (with different prefixes before the read number) and/or a folder of folders, each containing a paired read 1 and 2 file. This script will attempt to walk through the directory and all subdirectories of the input directory specified to find pairs.
...
S0_L002_R1_00X.fastq.gz and S0_L002_R2_00X.fastq.gz would be matched up reads. The directory is included as part of the matching filenames.

If an optional --barcode_indicator file is used, it is searched for in the same manner that the paired files are searched for, so if the default "_I1_" is used, then:
S0_L001_R1_001.fastq.gz and S0_L001_R2_001.fastq.gz would be matched up along with S0_L001_I1_001.fastq.gz as the barcode file input.

The output directory used for each call to join_paired_ends.py uses the base name of the input read 1 fastq file (a single directory would be problematic since the output names for join_paired_ends.py can be the same for different calls). Use the parameter --include_input_dir_path to also include the input directory name in the output directory path, and the --remove_filepath_in_name can be used in this case.
...
script_info['script_usage'].append(("""Example of an input folder of paired-up files (by filename, with the default _R1_ and _R2_ contained the forward and reverse reads filenames, respectively):""",
                                    """The input folder is named 'input_paired_files', the output directory is 'output_folder', and an optional parameter file is passed with -p. This file can specify an optional parameter for join_paired_ends.py, such as 'join_paired_ends:pe_join_method SeqPrep'""",
                                    """%prog -i input_paired_files/ -o output_folder/ -p qiime_parameters.txt"""))
script_info['script_usage'].append(("""Example of an input folder of folders (with the filenames having _forward_ and _reverse_ contained the forward and reverse read filenames, respectively):""",
                                    """The input folder is named 'input_folders', the output directory is 'output_folder'. Additionally, the individual folder names are included in the output folder names, but not the filenames, and a matching barcode (indicated by 'barcode') fastq file is included""",
                                    """%prog -i input_folders/ -o output_folder/ --read1_indicator '_forward_' --read2_indicator '_reverse_' --include_input_dir_path --remove_filepath_in_name -b --barcode_indicator '_barcode_'"""))

script_info['output_description']="""The commands will be executed by the script by default. The commands can be printed with the -w option, so one could write these directly to an output file by calling multiple_join_paired_ends.py X > Y where X are the input parameters, and Y is an output text file. A log file will also be generated."""
    ...
    if opts.parameter_fp:
        try:
            ...
            raise IOError("Can't open parameters file (%s). Does it exist? Do you have read access?"
                % opts.parameter_fp)
        params = get_params_str(parse_qiime_parameters(parameter_f)['join_paired_ends'])
        parameter_f.close()
    else:
        params = ""
