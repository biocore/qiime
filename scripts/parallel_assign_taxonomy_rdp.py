...
script_info['script_usage'].append(
    ("Example",
     "Assign taxonomy to all sequences in the input file (-i) using the RDP "
     "classifier and write the results (-o) to $PWD/rdp_assigned_taxonomy/. "
...
script_info['optional_options'] = [
    make_option('--rdp_classifier_fp', action='store',
                type='existing_filepath', default=rdp_classifier_fp,
                help='full path to rdp classifier jar file [default: '
                     '%default]'),
    make_option('-c', '--confidence', action='store',
                type='float', default=0.80,
                help='Minimum confidence to record an assignment '
                     '[default: %default]'),
    make_option('-t', '--id_to_taxonomy_fp', action='store',
                type='existing_filepath', default=default_id_to_taxonomy_fp,
                help='full path to id_to_taxonomy mapping file [default: %s]'
                     % default_id_to_taxonomy_fp),
    make_option('-r', '--reference_seqs_fp', action='store',
                help='Ref seqs to rdp against. [default: %s]'
    ...
    make_option('--rdp_max_memory', default=4000, type='int',
                help='Maximum memory allocation, in MB, for Java virtual '
                     'machine when using the rdp method.  Increase for large '
                     'training sets [default: %default]'),
