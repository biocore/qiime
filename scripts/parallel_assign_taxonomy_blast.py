...
script_info['script_usage'].append(
    ("Example",
     "Assign taxonomy to all sequences in the input file (-i) using BLAST with"
     " the id to taxonomy mapping file (-t) and reference sequences file (-r),"
     " and write the results (-o) to $PWD/blast_assigned_taxonomy/. ALWAYS "
     ...
     "%prog -i $PWD/inseqs.fasta -t $PWD/id_to_tax.txt -r $PWD/refseqs.fasta "
     "-o $PWD/blast_assigned_taxonomy/"))
...
script_info['optional_options'] = [
    make_option('-r', '--reference_seqs_fp', type='existing_filepath',
                help='Ref seqs to blast against.  Must provide either '
                     '--blast_db or --reference_seqs_db for assignment with '
                     'blast [default: %s]' % default_reference_seqs_fp,
    ...
    make_option('-e', '--e_value', type='float', default=0.001,
                help='Maximum e-value to record an assignment, only used for '
                     'blast method [default: %default]'),
    make_option('-B', '--blastmat_dir', action='store', type='string',
                help='full path to directory containing blastmat file '
                     '[default: %default]',
    ...
    script_info['optional_options'].append(
        make_option('-t', '--id_to_taxonomy_fp', action='store',
                    ...
                    help='full path to id_to_taxonomy mapping file [default:'
                         ' %s]' % default_id_to_taxonomy_fp,
                    default=default_id_to_taxonomy_fp))
else:
    script_info['required_options'].append(
        make_option('-t', '--id_to_taxonomy_fp', action='store',
                    type='existing_filepath',
                    help='full path to id_to_taxonomy mapping file '
                         '[REQUIRED]'))
