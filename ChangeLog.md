...
* Changed default parameters for uclust-based OTU picking: ``max_accepts`` is now 1 (was 8), ``max_rejects`` is now 8 (was 500), ``step_words`` is now 8 (was 12), and ``word_length`` is now 8 (was 12). These changes greatly reduce runtime, with minimal effect on the results. See Rideout et al., 2014 [PeerJ pre-print here](...waiting on link...).
* Disabled the prefilter by default in pick_open_reference_otus.py. This changes greatly reduces runtime, with minimal effect on the results. See Rideout et al., 2014 [PeerJ pre-print here](...waiting on link...).
