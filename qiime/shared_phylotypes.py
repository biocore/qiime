...
def calc_shared_phylotypes(otu_table, reference_sample=None):
    ...
    num_samples = len(otu_table.ids())
    result_array = zeros((num_samples, num_samples), dtype=int)
    for i, samp1_id in enumerate(otu_table.ids()):
        for j, samp2_id in enumerate(otu_table.ids()[:i + 1]):
            if reference_sample:
    ...
    return format_distance_matrix(otu_table.ids(), result_array) + "\n"
