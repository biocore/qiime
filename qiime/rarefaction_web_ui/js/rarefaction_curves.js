...
function plotLines(series, seriestitles, xaxisvals, gtitle, legend){
    ...
    chart.draw(data, {width: 800, height: 400, lineSize: .5, pointSize: 1.5, legend: legend, colors: colours, title: gtitle, titleX: 'Sequences Per Sample', titleY: 'Diversity Measure'});
}

function loadDiversityMeasures() {
	var filenms = ['PD_whole_tree_feces.txt','chao1_feces.txt'];
...
function loadMapping() {
	...
	var mappingfl = getFile("Ley_Backhed_Feces_Mapping_v1_barcodes.txt");
...
function collapseSeries(option) {
	...
	for(var i = 0; i < categoryOps[option].length; i++) // ie male or female so this would go two times
	{
		...
		newSeries.push(currAve);
		//console.log(currAve)
		//console.log(">>>" + categoryOps[option][i])
	}
	var newXaxis = new Array();
	for(var p = 0; p < data[3].length; p++)
	{
...
function makeOpsPanel(categories, catops, matrix) {
	...
	for(var i = 0; i < categories.length-1; i++)
	{
		...
		//htmllines += "<a class=\"ops\" onMouseOver=\"javascript:collapseSeries(\'"+categories[i]+"\');\" href=\"javascript:showHide(\'"+categories[i]+"\');\">"+categories[i]+"</a>";
