...
class CoreDiversityAnalyses(WorkflowCommand):
    """
    """
    _brief_description = """A workflow for running a core set of QIIME diversity analyses."""
    _script_description = """This script plugs several QIIME diversity analyses together to form a basic workflow beginning with a BIOM table, mapping file, and optional phylogenetic tree. 
...
"""
    _script_usage = [("","Run diversity analyses at 20 sequences/sample, with categorical analyses focusing on the SampleType and day categories. ALWAYS SPECIFY ABSOLUTE FILE PATHS (absolute path represented here as $PWD, but will generally look something like /home/ubuntu/my_analysis/).","%prog -i $PWD/otu_table.biom -o $PWD/core_output -m $PWD/map.txt -c SampleType,day -t $PWD/rep_set.tre -e 20")]
    _script_usage_output_to_remove = ['$PWD/core_output']
    ...
    _input_file_parameter_ids = ['input_biom_fp','mapping_fp',
                                 'parameter_fp','tree_fp']
    ...
    def run_command(self, 
        ...
        input_biom_fp = options['input_biom_fp']
        output_dir = options['output_dir']
        categories = options['categories']
        if categories != None:
            categories = categories.split(',')
        tree_fp = options['tree_fp']
        mapping_fp = options['mapping_fp']
        verbose = options['verbose']
        parallel = options['parallel']
        sampling_depth = options['sampling_depth']
        nonphylogenetic_diversity = options['nonphylogenetic_diversity']
        print_only = options['print_only']
        suppress_taxa_summary = options['suppress_taxa_summary']
        suppress_beta_diversity = options['suppress_beta_diversity']
        suppress_alpha_diversity = options['suppress_alpha_diversity']
        suppress_otu_category_significance = options['suppress_otu_category_significance']
    
        if options['parameter_fp'] != None:
            params = parse_qiime_parameters(open(options['parameter_fp'],'U'))
        else:
