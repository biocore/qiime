...
class SplitLibrariesFastq(WorkflowCommand):
    """
    """
    _brief_description = """This script performs demultiplexing of Fastq sequence data where barcodes and sequences are contained in two separate fastq files (common on Illumina runs)."""
    _script_description = """"""
    _script_usage = [("Demultiplex and quality filter (at Phred Q20) one lane of Illumina fastq data and write results to ./slout_q20.","","%prog -i lane1_read1.fastq.gz -b lane1_barcode.fastq.gz --rev_comp_mapping_barcodes -o slout_q20/ -m map.txt -q20"),
    ("Demultiplex and quality filter (at Phred Q20) one lane of Illumina fastq data and write results to ./slout_q20. Store trimmed quality scores in addition to sequence data.","","%prog -i lane1_read1.fastq.gz -b lane1_barcode.fastq.gz --rev_comp_mapping_barcodes -o slout_q20/ -m map.txt --store_qual_scores -q20"),
    ("Demultiplex and quality filter (at Phred Q20) two lanes of Illumina fastq data and write results to ./slout_q20.","","%prog -i lane1_read1.fastq.gz,lane2_read1.fastq.gz -b lane1_barcode.fastq.gz,lane2_barcode.fastq.gz --rev_comp_mapping_barcodes -o slout_q20/ -m map.txt,map.txt --store_qual_scores -q20"),
    ("Quality filter (at Phred Q20) one non-multiplexed lane of Illumina fastq data and write results to ./slout_single_sample_q20.","","%prog -i lane1_read1.fastq.gz --sample_id my.sample -o slout_single_sample_q20/ -m map_not_multiplexed.txt -q20 --barcode_type 'not-barcoded'"),
    ("Quality filter (at Phred Q20) two non-multiplexed lanes of Illumina fastq data and write results to ./slout_single_sample_q20.","","%prog -i lane1_read1.fastq.gz,lane2_read1.fastq.gz --sample_id my.sample -o slout_single_sample_q20/ -m map_not_multiplexed.txt -q20 --barcode_type 'not-barcoded'")]
    ...
    def run_command(self, 
        ...
        sequence_read_fps = options['sequence_read_fps']
        barcode_read_fps = options['barcode_read_fps']
        sample_id = options['sample_id']
        mapping_fps = options['mapping_fps']
        phred_quality_threshold = options['phred_quality_threshold']
        retain_unassigned_reads = options['retain_unassigned_reads']
        min_per_read_length_fraction = options['min_per_read_length_fraction']
        max_bad_run_length = options['max_bad_run_length']
        rev_comp = options['rev_comp']
        rev_comp_barcode = options['rev_comp_barcode']
        rev_comp_mapping_barcodes = options['rev_comp_mapping_barcodes']
        seq_max_N = options['sequence_max_n']
        start_seq_id = options['start_seq_id']
        # NEED TO FIX THIS FUNCTIONALITY - CURRENTLY READING THE WRONG FIELD
        filter_bad_illumina_qual_digit = False #options['filter_bad_illumina_qual_digit
        store_qual_scores = options['store_qual_scores']
        store_demultiplexed_fastq = options['store_demultiplexed_fastq']
        barcode_type = options['barcode_type']
        max_barcode_errors = options['max_barcode_errors']
        ...
        if options['last_bad_quality_char'] != None:
