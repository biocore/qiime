...
class CheckIdMap(WorkflowCommand):
    """
    """
    _brief_description = """Checks user's metadata mapping file for required data, valid format"""
    _script_description = """Specifically, we check that:
    ...
    """
    _script_usage = [("""Example:""","""Check the Fasting_Map.txt mapping file for problems, supplying the required mapping file, and output the results in the check_id_map_output directory""","""%prog -m Fasting_Map.txt -o check_id_map_output""")]
    _script_usage_output_to_remove = []
    _output_description = """A log file, html file, and corrected_mapping.txt file will be written to the current output directory."""
    _required_options = [
    ...
    def run_command(self, 
        ...
        variable_len_barcodes = options['variable_len_barcodes']
        output_dir = options['output_dir'] + "/"
        char_replace = options['char_replace']
        disable_primer_check = options['disable_primer_check']
        added_demultiplex_field = options['added_demultiplex_field']
