...
def get_sample_individual_info(mapping_data, header, individual_column,
    ...
    if timepoint_zero_column in header:
        ...
        raise ValueError(
            "The category mapping file must have a column called reference_sample. Please see the script documentation for more details.")
    if individual_column in header:
        individual_index = header.index(individual_column)
    else:
        raise ValueError(
            "The category mapping file must have a column called individual. Please see the script documentation for more details.")
...
def make_new_otu_counts(otu_table, sample_to_subtract, samples_from_subject):
    ...
    """
    new_sample_ids = sorted(sample_to_subtract.keys())
    ...
    for index1, otu in enumerate(otu_table.ObservationIds):
        for index2, sample in enumerate(new_sample_ids):
            ...
            if tpz_sample in otu_table.SampleIds:
                ...
                raise ValueError(
                    "There are samples in the category mapping file that are not in the otu table, such as sample: " +
                    tpz_sample +
                    ". Removing these samples from the category mapping file will allow you to proceed.")
            # get the new count as the relative abundance of the otu at
            # the later timepoint minus the relative abundance at timepoint
            # zero
