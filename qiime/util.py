...
def qiime_blast_seqs(seqs,
    ...
    seqs: a list (or object with list-like interace) of (seq_id, seq)
     tuples (e.g., the output of fasta_parse)
...
def split_fasta_on_sample_ids(seqs):
    """ yields (sample_id, seq_id, seq) for each entry in seqs
        ...
        seqs: (seq_id,seq) pairs, as generated by fasta_parse
...
def count_seqs(fasta_filepath, parser=fasta_parse):
...
def count_seqs_from_file(fasta_file, parser=fasta_parse):
...
def subsample_fasta(input_fasta_fp,
    ...
    for label, seq in fasta_parse(input_fasta):
