...
QIIME is well-tested: we make automated software testing a high priority, as you can see by reviewing the ``Qiime/tests/`` `here <http://qiime.svn.sourceforge.net/viewvc/qiime/trunk/tests/>`_. After installing QIIME, users can run the automated test suite by changing to the ``Qiime/tests`` directory and running::
